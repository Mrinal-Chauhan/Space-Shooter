...
if True:  #Variables!!
    ...
    spaceship_velocity,bullet_velocity,ast_velocity,bg_velocity = 4,3,2,1
    bullets,asteroids,backgrounds = [],[],[]
    ast_count = 0
    ast_onscreen = 5
    bg_count = 0
...
def load():
    global bg,spaceship_img,ast_img,font_score,font_endscreen1,font_endscreen2,font_startscreen,font_startscreen2,heart_img,laser_sound
    bg = pygame.image.load('data/bg.png')
    bg = pygame.transform.scale(bg,screen_dimen) 
...
def bullet_controls():
    global current_y, counter_collision,bullet_velocity
    for bullet in bullets:
...
def death():
    global death_counter,highscore
    global score
...
def bg_move():
    global bg_count
    if bg_count == 0:
        back_ground = pygame.Rect(0,0,900,600) 
        backgrounds.append(back_ground)
        bg_count += 1
    elif bg_count == 1:
        back_ground = pygame.Rect(0,-600,900,600)
        backgrounds.append(back_ground)
        bg_count += 1
    for background in backgrounds:
        screen.blit(bg,(background.x,background.y))
        background.y += bg_velocity
        if background.y >= 600:
